...
def align(model, data, align_or_not=True):
...
def evaluate_ate(first_list, second_list, align_or_not=True, plot="", _args=""):
    # parse command line
    ...
    rot, trans, trans_error = align(second_xyz, first_xyz, align_or_not)
    ...
    if args.save_associations:
        ...
        file.write("\n".join(["%f %f %f %f %f %f %f %f" % (a, x1, y1, z1, b, x2, y2, z2) for (a, b), (x1, y1, z1), (x2, y2, z2) in
...
def evaluate(poses_gt, poses_est, plot, align_or_not=True):
    ...
    results = evaluate_ate(poses_gt, poses_est, align_or_not, plot)
    return results
...
def pose_evaluation(poses_gt, poses_est, scale, path_to_save, i, align_or_not=True, img='pose', name='output.txt'):
    N = len(poses_est)
    ...
    results = evaluate(poses_gt, poses_est, plot=plt_path, align_or_not=align_or_not)
    results['Name'] = i
